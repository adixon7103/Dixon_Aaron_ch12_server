...
def fetch_links():
    ...
    try:
        ...
        if external_links:
            _external_links.set(tuple(external_links))
            sb('External links found: {}'.format(len(external_links)))
        else:
...
if __name__ == "__main__": # execute logic if run directly
    ...
    _root.title('Aaron Dixon SEO app')
    ...
    _fetch_btn.grid(row=0, column=3, sticky=W, padx=5)
    ...
    _external_links = StringVar()
    _external_links_listbox = Listbox(
        _img_frame, listvariable=_external_links, height=6, width=40)
    _external_links_listbox.grid(row=1, column=0, sticky=(E, W), pady=5)
